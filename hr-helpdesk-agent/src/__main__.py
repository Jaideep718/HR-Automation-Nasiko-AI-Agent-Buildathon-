"""
HR Helpdesk Agent - FastAPI Server
Handles A2A JSON-RPC requests for HR policy questions and issue escalation.
"""
import logging
import uuid
import click
...
logger = logging.getLogger("hr-helpdesk-agent")

# Initialize FastAPI app
app = FastAPI(
    title="HR Helpdesk Agent",
    description="An AI-powered HR helpdesk agent that answers policy questions and escalates complex issues.",
    version="1.0.0"
)
...
async def handle_rpc(request: JsonRpcRequest):
    ...
    if request.method == "message/send":
        try:
            ...
            logger.info(f"Received HR query: {input_text[:100]}... (Session: {session_id})")
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Method '{request.method}' not found. Supported methods: message/send"
        )
...
if __name__ == "__main__":
    ...
    def main(host: str, port: int):
        """Start the HR Helpdesk Agent server."""
        logger.info(f"Starting HR Helpdesk Agent on {host}:{port}")
